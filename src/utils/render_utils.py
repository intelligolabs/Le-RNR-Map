...
def add_agent_view_on_w(w_im, Rt, coordinate_scale, map_size, agent_color=(255,0,0),
                        agent_size=4, transparency=0.7, view_color=(255,255,255), view_size=20, target_position=None):
    ...
    #target_position=None -> map_creation
    #target_position!=None -> query_map
    if target_position is not None:
        for pos in target_position:
            if pos is not None:
                x, y = pos
                cv2.drawMarker(w_im, (int(x), int(y)), (255, 255, 255), markerType=cv2.MARKER_STAR, markerSize=10, thickness=1)
